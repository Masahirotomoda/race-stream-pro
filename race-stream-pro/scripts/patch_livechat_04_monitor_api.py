...
write(
  "apps/web/app/api/reservations/[id]/monitor/route.ts",
...
async function getTwitchAppAccessToken() {
  ...
  const now = Date.now();
  if (twitchAppTokenCache && twitchAppTokenCache.expiresAt > now + 60_000) return twitchAppTokenCache.token;
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!admin && reservation.user_id !== user.id) return NextResponse.json({ ok: false, message: "Forbidden" }, { status: 403 });
  ...
  if (youtubeVideoId) {
    ...
    if (!ytKey) {
      ...
      if (res.ok) {
        ...
        const lbc = snippet?.liveBroadcastContent ?? null;
