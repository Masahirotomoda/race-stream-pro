...
write("apps/web/app/api/reservations/[id]/youtube-chat/route.ts", r'''import { NextResponse } from "next/server";
...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Reservation ownership/admin check
  ...
  if (tErr || !tok || (!tok.access_token && !tok.refresh_token)) {
  ...
  // 1) activeLiveChatId from videos.list (OAuth)
